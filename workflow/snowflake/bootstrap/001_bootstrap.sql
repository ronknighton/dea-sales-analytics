...
GRANT CREATE STAGE ON SCHEMA SALES_ANALYTICS_PIPELINE.GOLD TO ROLE SALES_ANALYTICS_DEPLOY_ROLE;
GRANT CREATE STREAMLIT ON SCHEMA SALES_ANALYTICS_PIPELINE.GOLD TO ROLE SALES_ANALYTICS_DEPLOY_ROLE;
-- Added when the Streamlit dashboard was built: snow streamlit deploy
-- creates a backing stage (if one doesn't already exist) plus the
-- STREAMLIT object itself — both need explicit grants. Anyone
-- rebuilding this account from scratch needs both from the start;
-- anyone who already ran bootstrap needs to run these two lines
-- manually before their first Streamlit deploy.
